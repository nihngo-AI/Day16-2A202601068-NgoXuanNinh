...
# Cùng logic khớp-dòng như scorer, để "đã đúng trích dẫn" nghĩa y hệt.
...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if cited is not None and _supports(_norm_lines(cited.body), nclaim):
                continue  # trích dẫn đã đúng

            for doc in ctx.corpus.docs:
                if doc.body in observed and _supports(_norm_lines(doc.body), nclaim):
                    claim["doc_id"] = doc.doc_id   # GẮN LẠI, giữ nguyên text
                    break
            # không tìm được nguồn -> để `critic` xoá, đừng bịa doc_id
