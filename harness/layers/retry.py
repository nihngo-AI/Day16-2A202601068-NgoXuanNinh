...
class Retry(Middleware):
    ...
    def _degraded(self, result) -> bool:
        return (not result.ok) or (
            isinstance(result.content, str) and is_degraded(result.content)
        )
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        attempts = 1
        while attempts < self.max_attempts and self._degraded(result):
            if self._budget_exhausted(ctx):
                break
            result = call(name, args)
            attempts += 1
        ctx.state["retry_attempts"] = ctx.state.get("retry_attempts", 0) + (attempts - 1)
        return result  # trả sự thật, kể cả khi vẫn hỏng
