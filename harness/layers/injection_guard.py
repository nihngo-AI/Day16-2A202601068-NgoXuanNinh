...
def _strip_blocks(content: str) -> str:
    """Cách ly mọi khối độc: cắt từ BLOCK_START tới BLOCK_END (hoặc tới hết
    chuỗi nếu fetch bị cắt và thiếu mốc đóng), thay bằng PLACEHOLDER."""
...
class InjectionGuard(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        if not isinstance(content, str) or BLOCK_START not in content:
            return result
        return ToolResult(ok=result.ok, content=_strip_blocks(content), error=result.error)
